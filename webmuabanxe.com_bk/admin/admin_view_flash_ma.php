              ...
              <tr>
                <td height="285" valign="top" class="bg_center">
					...
					if(isset($HTTP_GET_VARS["start"])){
					 ...
					 $page_count =15; 
					 $cut_off = 10; 
					 $PHP_SELF ="admin_view_flash_ma.php";
					 include("inc/phantrang.php");
					 $query = "Select * FROM  $table $sort order by thu_tu ASC LIMIT $start, $page_count";
					 $result = mysql_query($query);
					 $tam=0;
					 ?>
								<form name=messageList ENCTYPE="multipart/form-data" method=post action="admin_edit_save.php?save=sort_ma_flash">
								...
								<table id="datatable" class="tbldata" width="100%" cellpadding=2 cellspacing=0 border=0>
									...
									<tr bgcolor='ffffff' onMouseOut=this.style.backgroundColor='#ffffff'; onmouseover=this.style.backgroundColor='#EAEAEA';>
									  ...
									  <td><a href="admin_view_flash.php?cat=<?=$rs["ma_files"]; ?>&start=0"><? echo"<img src=\"images/folder.gif\" border=\"0\" align=\"absmiddle\"/> "; ?><?=$rs["ten_ma_files"] ?></a></td>
									  <td><?=$rs["mo_ta"] ?></td>
									  <td><? 
									  		 if($suid==1)
											 $num_ = dem_max_dk("tb_flash","ma_files",$ma_files);
											 else
											 $num_ = dem_max_2dk("tb_flash","ma_files",$ma_files,"user",$ma_user_admin);
										     echo"$num_";
									      ?></td>
									  ...
									  <td colspan="2" align="center"><? if($num_==0) {?><a href="admin_view_flash_ma_edit.php?id=<?=$rs["id"]?>&ma_edit=ma_flash"><img src="images/edit.gif" alt="Edit" border="0"></a><? }?></td>
									  </tr>
									<? } ?>
									</table>
										<table width="100%" border="0" cellpadding="0" cellspacing="0" background="images/bg_icon.gif">
										  <tr>
											...
											<img src="images/delete.gif" onClick="delete_all_do('admin_delete_wait.php','?ma_del=ma_flash')" style="cursor: pointer" alt="<?=$view_item_buttom_xoa?>" align="absmiddle"/>
											</td>
										  </tr>
										</table>
