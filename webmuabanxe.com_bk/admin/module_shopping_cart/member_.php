        ...
        <!--DWLayoutTable-->
              ...
              <tr>
                ...
                <tr align="left" bgcolor="#DEDFDE"> 
                  <td height="29" colspan="10" valign="middle" class="tab_left_10">
				  <? $cat=$_GET["cat"];?>
					 <select name="trang_thai" onChange="MM_jumpMenu('parent',this,0)" size="1" id="select" style="FONT: 11px verdana,arial,helvetica;">
                       ...
                       <option value="member.php?cat=1&start=0" <? if($cat==1) echo"selected";?> >- Sort by Discount -</option>
					    <option value="member.php?cat=2&start=0" <? if($cat==2) echo"selected";?> >- Sort by Percent -</option>
						...
						<option value="member.php?cat=5&start=0" <? if($cat==5) echo"selected";?> >- Sort by Type -</option>
		    ...
		    <FORM style="MARGIN: 0px" name="form" action="edit.php?ma_edit=discount" method=post>
				 ...
				 <tr align="center" bgcolor="#EFEFEF"> 
                  <td height="31" colspan="10" align="right"  valign="middle"> <table width="100%" border="0" cellpadding="0" cellspacing="0">
  <!--DWLayoutTable-->
    <tr>
    ...
    <tr>
      <td height="1"></td>
				                ...
				                <tr>
				                  <td width="33" align="center" valign="middle" bgcolor="#EFEFEF" class="bg3"><!--DWLayoutEmptyCell-->&nbsp;</td> 
                  <td width="267" height="23" align="left" valign="middle" bgcolor="#EFEFEF" class="bg3">Full Name</td>
                  <td width="39" align="left" valign="middle" bgcolor="#EFEFEF" class="bg3">In</td>
                  <td width="42" align="left" valign="middle" bgcolor="#EFEFEF" class="bg3">Edit </td>
                  <td width="67" align="left" valign="middle" bgcolor="#EFEFEF" class="bg3">Mark</td>
                  <td width="83" align="left" valign="middle" bgcolor="#EFEFEF" class="bg3">Type</td>
                  <td width="78" align="left" valign="middle" bgcolor="#EFEFEF" class="bg3">Percent type </td>
                  <td width="111" align="left" valign="middle" bgcolor="#EFEFEF" class="bg3">Discount type </td>
                  <td width="71" align="left" valign="middle" bgcolor="#EFEFEF" class="bg3">Commission</td>
                <td width="26" align="center" valign="middle" bgcolor="#EFEFEF" class="bg3">Acti</td>
                </tr>
                 <? 
				...
				<input name="chkBuyOfferIds" type="hidden" id="chkBuyOfferIds" value="-1" checked>
                <tr>
                  <td width="33" align="center" valign="middle" bgcolor="#EFEFEF" class="S9"><input name="chkBuyOfferIds" type=checkbox id="chkBuyOfferIds" value="<? echo"$rs[id]"?>">
					</td> 
                  <td width="267" height="23" align="left" valign="middle" bgcolor="#EFEFEF" class="S9"><? echo"<a href=javascript:view_profile('$ma_user') title=$email>$rs[ten]</a>"; ?></td>
                  <td align="left" valign="middle" bgcolor="#EFEFEF" class="S9"><?
					if($code_introduction!=0)
					echo"<a href=javascript:view_profile('$code_introduction')>In</a>";
					else
					echo"In";
				  ?></td>
                  <td align="left" valign="middle" bgcolor="#EFEFEF" class="S9"><? echo"<a href=\"member_edit.php?ma_user=$ma_user\">Edit</a>";?></td>
                  <td width="67" align="left" valign="middle" bgcolor="#EFEFEF" class="S9"><input name="diem_<?=$i?>" value="<?=$rs["diem"]?>" type="text" style="width:60px;"></td>
                  <td width="83" align="left" valign="middle" bgcolor="#EFEFEF" class="S9">
				  <select name="type_<?=$i?>" size="1" id="select" class="bottom">
                    <option value="0" <? if($rs["user_type"]==0) echo"selected"; ?>>Partty</option>
					<option value="1" <? if($rs["user_type"]==1) echo"selected"; ?>>Member</option>
                    </select></td>
                  <td width="78" align="left" valign="middle" bgcolor="#EFEFEF" class="S9"><? 
			$table_="tb_shopping_cart_percent";
			$query_ = "Select * FROM  $table_ order by percent_value ASC";
			$result_ = mysql_query($query_);
			?>
                    <select name="percent_code_<?=$i?>" size="1" id="select" class="bottom">
                      <? while($rs_= mysql_fetch_array($result_))
				{
				$code_percent=$rs_["percent_code"];
				?>
                      <option value="<?=$code_percent?>" <? if($code_percent==$rs["code_percent"]) echo"selected"; ?>> <? echo "$rs_[percent_name] ($rs_[percent_value]) ";?> </option>
                      <? } ?>
                    </select></td>
                  <td width="111" align="left" valign="middle" bgcolor="#EFEFEF" class="S9"><input type="hidden" name="id_<?=$i?>" value="<?=$id?>">
                    <? 
				$table_="tb_shopping_cart_discount";
				$query_ = "Select * FROM  $table_ order by discount_value ASC";
				$result_ = mysql_query($query_);
				?>
                    <select name="discount_<?=$i?>" size="1" id="select" class="bottom">
                      <? while($rs_= mysql_fetch_array($result_))
				{
				...
				?>
                  ...
                  <td width="71" align="left" valign="middle" bgcolor="#EFEFEF" class="S9"><input name="percent_<?=$i?>" value="<?=$rs["total_percent"]?>" type="text" style="width:60px;"></td>
                <td width="26" align="center" valign="middle" bgcolor="#EFEFEF">
                  <? if($rs["kichhoat"]==1) echo"<span class=xanh>ON</span>"; else echo"<span class=do>OFF</span>";?>
                </td>
                </tr>
                <? }?>
                <tr align="center" bgcolor="#EFEFEF"> 
                  <td height="31" colspan="10" valign="middle"><table width="100%" border="0" cellpadding="0" cellspacing="0">
  <!--DWLayoutTable-->
    <tr>
    ...
    <tr>
      <td height="1"></td>
                ...
                <tr align="center" bgcolor="#DEDFDE"> 
                  <TD height=27 colspan="10" valign="middle" class="tab_lr_5"> <? echo"View $i of $num ";?></TD>
                </tr>
                </table>                  <? ?></td>
              </tr>
                          </table></td>
