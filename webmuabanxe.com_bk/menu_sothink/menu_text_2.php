...
<script language="javascript">
			...
			$i=0;
			$i_bg=0;
			$query = "Select * FROM  tb_bac_1 where trang in ('1','2','3','6','30','31','32') and activate=1 and activate_setup=1 order by thu_tu ASC ";
		  ...
		  switch($i_bg)
			...
			*/
			...
			?>
			stm_ai("p0i<?=$i?>",[0,"       <?=$ten?>       ","images/danh_muc/<?=$ten_anh?>","images/danh_muc/<?=$ten_anh_lon?>",-1,-1,0,"<?=$link?>","_self","","","","",0,0,0,"","",0,0,0,0,1,"#FFFFFF",1,"#ffffff",1,"","",3,3,0,0,"#fffff7","#FFFFFF","#FFFFFF","#FF9900","bold 12px arial","bold 12px arial",0,0]);
			/* [1,4,0,0,Day la border,Day la cach tren cach duoi,0,0,100... ]*/
			stm_bpx("p1","p0",[1,4,0,0,2,3,0,0,100,"progid:DXImageTransform.Microsoft.Wipe(GradientSize=1.0,wipeStyle=1,motion=forward,enabled=0,Duration=0.60)",5,"progid:DXImageTransform.Microsoft.Wipe(GradientSize=1.0,wipeStyle=1,motion=reverse,enabled=0,Duration=0.60)",4,50,0,0,"#3399ff","#fffff7"]);
			<?
			$j=0;
			...
			?>
				stm_aix("p2i<?=$j?>","p0i0",[0,"  <?=$ten_tp?>  ","","",-1,-1,0,"<?=$link?>","_self","","","","",0,0,0,"","",0,0,0,0,1,"<?=$mau_bg?>",0,"<?=$mau_bg_over?>",0,"","",3,3,0,0,"#fffff7","#ffffff","#FFFFFF","#FF9900","12px arial","12px arial",0,0]);
				stm_bpx("p3","p1",[1,2,0,0,1,1,0,0,100,"progid:DXImageTransform.Microsoft.Wipe(GradientSize=1.0,wipeStyle=1,motion=forward,enabled=0,Duration=0.60)",5,"progid:DXImageTransform.Microsoft.Wipe(GradientSize=1.0,wipeStyle=1,motion=reverse,enabled=0,Duration=0.60)",4,50,0,0,"#3399ff","#fffff7"]);
					...
					?>
					stm_aix("p3i<?=$k?>","p0i0",[0,"  <?=$ten_dv_ct?>  ","","",-1,-1,0,"<?=$link?>","_self","","","","",0,0,0,"","",0,0,0,0,1,"<?=$mau_bg?>",0,"<?=$mau_bg_over?>",0,"","",3,3,0,0,"#fffff7","#ffffff","#FFFFFF","#ffffff","11px tahoma","11px tahoma",0,0]);
					<? $k++; } ?>
				stm_ep();
			    <? $j++; } ?>
			stm_ep();
			...
			if($i< $num_menu)
			{?>
			stm_aix("p0i1","p0i0",[2," | ","images/linedoc.jpg","images/linedoc.jpg",2,21,0,"","_self","","","","",0,0,0,"","",0,0,0,0,1,"#ffffff",1,"#ffffff",1,"","",3,3,0,0,"#fffff7","#ffffff","#FFFFFF","#ffffff","11px tahoma","11px tahoma",0,0]);
			<?  } } ?>
			stm_em();
  </script>