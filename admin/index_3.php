        ...
        <!--DWLayoutTable-->
              <tr class="M">
				...
				$bac=3;
				 ...
				 $table="tb_bac_3";
				 if($ma_user_admin==1)
				   $query = "Select * FROM  $table where ma_bac_2=$cat ";
				 else
				   $query = "Select * FROM  $table where ma_bac_2=$cat and activate_setup=1 ";
				 
				 $query.=$sort;
				 $result = mysql_query($query);
				 $tam=0;
				 $num = mysql_num_rows($result);
				 $PHP_SELF="index_3.php";
				 $page_count =30; 
				 $cut_off = 5; 
				 $start=$HTTP_GET_VARS["start"];
				 include("inc/phantrang.php");
				 if($ma_user_admin==1)
				   $query2 = "Select * FROM  $table where ma_bac_2=$cat ";
				 else
				   $query2 = "Select * FROM  $table where ma_bac_2=$cat and activate_setup=1 ";
				 $query2.=$sort;
				 $query2.=" ORDER BY activate_setup DESC, thu_tu ASC LIMIT $start, $page_count";
				 $result2 = mysql_query($query2);
				 
				 echo"<a href=\"index.php?cat=0&lg=$lg&alpha=All&start=0\"><img src=\"images/home.gif\" border=0> $menu_trang_chu</a>";?> » <a href="index_2.php?cat=<?=$ma_bac_1?>&lg=<?=$lg?>&alpha=All&start=0"> <? echo gia_tri_mot_cot("tb_bac_1","ma_bac_1",$ma_bac_1,"ten_$lg")?></a> »  <? echo gia_tri_mot_cot("tb_bac_2","ma_bac_2",$cat,"ten_$lg")?></td>
             </tr>
              <tr>
                <td height="285" valign="top" class="bg_center">
  		    <form name=messageList ENCTYPE="multipart/form-data" method=post action="admin_edit_save.php?save=sort_s_s">
		   ...
		   <input type="hidden" name="start" value="<?=$start?>">
			<input type="hidden" name="lg" value="<?=$lg?>">		   
			 <? 
					 ...
					 $page_view=gia_tri_mot_cot_2dk("tb_setup","ma_dm","$rs[trang]","ma_bac","3","file_view");
					 $qr_view=lay_id($page_view,1);
					 $page_view=str_replace('|'.$qr_view.'|',"",$page_view);

					 $tb_detail=gia_tri_mot_cot_2dk("tb_setup","ma_dm","$rs[trang]","ma_bac","3","tb");
					...
					if($rs["activate_setup"]==1)
					 ...
					 if($ma_user_admin==1)
						 ...
						 $num_ = dem_max_2dk("tb_bac_4 ","ma_bac_3","$ma_bac_3","activate_setup",1);
						 
					 $link="$page_view?cat=$ma_bac_3&bac=$bac&start=0&lg=$lg&$qr_view";
				     $link_in="index_4.php?cat=$ma_bac_3&alpha=All&start=0&lg=$lg";
					if($num_>0)
					  $link_icon=$link_in;
					else
					  $link_icon=$link;
				...
				<td align=center valign=middle><input id="1" type="checkbox" name="Mid" value="<?=$ma_bac_3?>"></td>
				<td><? echo "<a href=\"$link_icon\" ><img src=\"images/$folder\" border=\"0\" align=\"absmiddle\"/></a> "; ?>
				  <input name="ten_<?=$tam?>" type="text"  size="50" class="input" value="<?=$rs["ten_$lg"]?>" />
				 </td>
				...
				<td><? 
			   echo"<span class=\"lot\"><A href=\"javascript:openWin('inc/folder_content.php?id=$rs[id]&bac=3&lg=$lg','smilies','toolbar=0,location=0,status=0,menubar=0,scrollbars=1,resizable=1,width=500,height=350')\" title=\"$note_statictics\"><b>?</b></a></span>";
