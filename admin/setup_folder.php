		...
		if(strstr($_SERVER["QUERY_STRING"],"update"))
		   {
			...
			$file_add=rep($HTTP_POST_VARS["add_$i"]);
			$file_edit=rep($HTTP_POST_VARS["edit_$i"]);
			$file_view=rep($HTTP_POST_VARS["view_$i"]);
			$file_view_web=rep($HTTP_POST_VARS["view_web_$i"]);
			$tb=rep($HTTP_POST_VARS["tb_$i"]);
			$activate=rep($HTTP_POST_VARS["activate_$i"]);
			$bac=rep($HTTP_POST_VARS["bac_$i"]);
			$thu_tu=rep($HTTP_POST_VARS["thu_tu_$i"]);

			$table_edit="tb_setup";
			$query_edit = "UPDATE $table_edit SET ten_dm='$ten_dv',file_add='$file_add',file_edit='$file_edit',file_view='$file_view',tb='$tb',file_view_web='$file_view_web',ma_bac='$bac',activate='$activate',thu_tu='$thu_tu'";
...
<table id="datatable" class="tbldata" width="100%" cellpadding=2 cellspacing=0 border=0>
	...
	<td><select name="bac_<?=$tam?>"  class="selected" id="trang">
	...
	<td><input type="hidden" name="id_<?=$tam?>" value="<?=$rs["id"]?>" />
              <br />
              <input name="ten_<?=$tam?>" type="text" class="input" style="width:350px;" value="<?=$rs["ten_dm"];?>" />
              <br />
              <input name="add_<?=$tam?>" type="text" class="input" style="width:350px;" value="<?=$rs["file_add"];?>" />
              <br />
              <input name="edit_<?=$tam?>" type="text" class="input" style="width:350px;" value="<?=$rs["file_edit"];?>" />
              <br />
              <input name="view_<?=$tam?>" type="text" class="input" style="width:350px;" value="<?=$rs["file_view"];?>" />
              <br />
              <input name="tb_<?=$tam?>" type="text" class="input" style="width:350px;" value="<?=$rs["tb"];?>" />
              <br />
			  <input name="view_web_<?=$tam?>" type="text" class="input" style="width:350px;" value="<?=$rs["file_view_web"];?>" />
              <br /></td>
	...
	<td align="center"><select name="activate_<?=$tam?>"  class="selected" id="activate">
	...
	?><input type="button" name="Submit42" value="Update" onClick="update_all_do('<?=$form_action?>','?update')" class="nut_nhan">
        ...
        <p><span style="font-weight: bold">New Folder:</span><br>
            <br>
