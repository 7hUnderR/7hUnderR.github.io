...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <td width="85%" valign="top"><table width="100%" border="0" cellpadding="0" cellspacing="0">
		...
		$bac=4;
		 ...
		 $table="tb_bac_4 ";
		 if($ma_user_admin==1)
		   $query = "Select * FROM  $table where ma_bac_3=$cat ";
		 else
		   $query = "Select * FROM  $table where ma_bac_3=$cat and activate_setup=1 ";
		   
		 $query.=$sort;
		 $result = mysql_query($query);
		 $tam=0;
		 $num = mysql_num_rows($result);
		 $PHP_SELF="index_4.php";
		 $page_count =30; 
		 $cut_off = 5; 
		 $start=$HTTP_GET_VARS["start"];
		 include("inc/phantrang.php");
		 if($ma_user_admin==1)
		    $query2 = "Select * FROM  $table where ma_bac_3=$cat ";
		 else
		    $query2 = "Select * FROM  $table where ma_bac_3=$cat and activate_setup=1";
    ...
    »  <? echo gia_tri_mot_cot("tb_bac_3","ma_bac_3",$cat,"ten_$lg")?></td>
             </tr>
              <tr>
                <td height="285" valign="top" class="bg_center">
		 <form name=messageList ENCTYPE="multipart/form-data" method=post action="admin_edit_save.php?save=sort_s_s_s">
		  ...
		  <? 
			 ...
			 $page_view=gia_tri_mot_cot_2dk("tb_setup","ma_dm","$rs[trang]","ma_bac","4","file_view");
			 $qr_view=lay_id($page_view,1);
			 $page_view=str_replace('|'.$qr_view.'|',"",$page_view);
			 
			 $tb_detail=gia_tri_mot_cot_2dk("tb_setup","ma_dm","$rs[trang]","ma_bac","4","tb");
		   ...
		   if($ma_user_admin==1)
		     ...
		     $num_ = dem_max_2dk("tb_bac_5","ma_bac_4","$ma_bac_4","activate_setup",1);
			 
			 $link="$page_view?cat=$ma_bac_4&bac=$bac&start=0&lg=$lg&$qr_view";
			 $link_in="index_5.php?cat=$ma_bac_4&alpha=All&start=0&lg=$lg";
			if($num_>0)
			  $link_icon=$link_in;
			else
			  $link_icon=$link;
       ...
       <tr <?=$style?> bgcolor='ffffff' onMouseOut=this.style.backgroundColor='#ffffff'; onmouseover=this.style.backgroundColor='#EAEAEA';>
				<td align=center valign=middle><input id="1" type="checkbox" name="Mid" value="<?=$ma_bac_4?>"></td>
				...
				<td><? 
			 echo"<span class=\"lot\"><A href=\"javascript:openWin('inc/folder_content.php?id=$rs[id]&bac=4&lg=$lg','smilies','toolbar=0,location=0,status=0,menubar=0,scrollbars=1,resizable=1,width=500,height=350')\" title=\"$note_statictics\"><b>?</b></a></span>";	
            ...
            <input name="thu_tu_<?=$tam?>" type="text"  class="input" size="3" value="<?=$rs["thu_tu"]?>" onkeydown="AcceptNumbersOnly();"></td>
