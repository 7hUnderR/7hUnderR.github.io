                ...
                <td height="285" valign="top" class="bg_center">
					...
					if(isset($HTTP_GET_VARS["start"])){
					 ...
					 if($alpha=="All")
						 ...
						 $sort="where ten_ma_images like '".$alpha."%'";
					 ...
					 $page_count =30; 
					 $cut_off = 10; 
					 $PHP_SELF ="admin_view_images_ma.php";
					 ...
					 $tam=0;
					 ?>
								<form name=messageList ENCTYPE="multipart/form-data" method=post action="admin_edit_save.php?save=sort_ma_images">
 					<? include("inc/inc_sort.php"); ?>
								<table id="datatable" class="tbldata" width="100%" cellpadding=2 cellspacing=0 border=0>
									...
									<thead>
									  ...
									  <th width="12%" colspan="2" id="senderheader"><?=$main_tieu_de_update?></th>
									  </tr>
									  </thead>
									<tbody>
									...
									<tr bgcolor='ffffff' onMouseOut=this.style.backgroundColor='#ffffff'; onmouseover=this.style.backgroundColor='#EAEAEA';>
									  <td height="24" align=center valign=middle><input id="1" type="checkbox" name="Mid" value="<?=$ma_images?>" ></td>
									  <td><a href="admin_view_images.php?cat=<?=$rs["ma_images"]; ?>&start=0"><? echo"<img src=\"images/folder.gif\" border=\"0\" align=\"absmiddle\"/> "; ?><?=$rs["ten_ma_images"] ?></a></td>
									  <td><?=$rs["mo_ta"] ?></td>
									  <td><? 
									  if($suid==1)
									  	$num_ = dem_max_dk("tb_images","ma_images",$ma_images);
									  else
									  	$num_ = dem_max_2dk("tb_images","ma_images",$ma_images,"user",$ma_user_admin);
									  echo"$num_";
									      ?></td>
									  ...
									  <td colspan="2" align="center"><? if($num_==0) {?><a href="admin_view_images_ma_edit.php?id=<? echo $rs["id"]?>&ma_edit=ma_images"><img src="images/edit.gif" alt="Edit" border="0"></a><? }?></td>
								  </tr>
									<? } ?>
									</table>
										<table width="100%" border="0" cellpadding="0" cellspacing="0" background="images/bg_icon.gif">
										  <tr>
											...
											<img src="images/delete.gif" onClick="delete_all_do('admin_delete_wait.php','?ma_del=ma_images')" style="cursor: pointer" alt="<?=$view_item_buttom_xoa?>" align="absmiddle"/>
											</td>
										  </tr>
										</table>										  										 
